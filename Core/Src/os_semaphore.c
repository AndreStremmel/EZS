...
static OS_Result_t prv_Take(OS_Semaphore_t *sem, uint32_t u32TimeoutTicks, uint8_t u8NonBlocking)
{
    ...
    for (;;)
    {
        ...
        {
            OS_Trace_Record2(OS_TRACE_EVT_SEM_TAKE_TRY, sem->u8TraceId, u8Idx);
        }
        ...
        {
            sem->u8Count--;
            OS_Trace_Record3(OS_TRACE_EVT_SEM_TAKE_OK, sem->u8TraceId, u8Idx, sem->u8Count);
        ...
        if (u32TimeoutTicks != OS_WAIT_FOREVER)
        {
            ...
            if (u32Remaining == 0u)
            {
                OS_Trace_Record2(OS_TRACE_EVT_SEM_TAKE_TIMEOUT, sem->u8TraceId, u8Idx);
                OS_vExitCritical(u32PriMask);
                return OS_TIMEOUT;
            }
            // Mit (Rest-)Zeit blockieren
            sem->u32WaitMask |= u32Bit;
            OS_Trace_Record3(OS_TRACE_EVT_SEM_TAKE_BLOCK, sem->u8TraceId, u8Idx, u32Remaining);
        ...
        else
        {
            ...
            sem->u32WaitMask |= u32Bit;
            OS_Trace_Record3(OS_TRACE_EVT_SEM_TAKE_BLOCK, sem->u8TraceId, u8Idx, OS_WAIT_FOREVER);
...
void OS_Semaphore_Give(OS_Semaphore_t *sem)
{
    ...
    {
        sem->u8Count++;
        OS_Trace_Record3(OS_TRACE_EVT_SEM_GIVE_OK, sem->u8TraceId, u8Task, sem->u8Count);
    }
    else
    {
        // Binaere Semaphore war bereits gesetzt -> Give wird verworfen
        OS_Trace_Record2(OS_TRACE_EVT_SEM_GIVE_IGNORED, sem->u8TraceId, u8Task);
    }
