...
{
    // Der SysTick laeuft bereits - beide Zeiger atomar setzen, damit der
    // Boot-Guard nicht mit halb initialisiertem Zustand oeffnet.
    __disable_irq();
    g_pNextTask    = &tasks[0];
...
{
    // Boot-Guard (siehe Scheduler_vCountdown): vor Scheduler_vInit()
    // liefert der Aufruf NULL - der SysTick-Handler vergleicht dann
    // NULL != NULL und loest keinen PendSV aus.
...
void Scheduler_vCountdown(void) {
    // Boot-Guard: der SysTick laeuft schon ab HAL_Init(), der Scheduler
    // aber erst ab Scheduler_vInit(). Vorher: nichts tun.
    if (g_pCurrentTask == (TCB_sctTCB_t*)0) {
        return;
    }
    for (uint8_t i = 0; i < NUM_TASKS; i++) {
...
{
    (void)u32SysViewCause;   /* nur fuer Trace - s. os_trace_config.h */
    uint8_t u8Idx = Scheduler_u8GetCurrentTaskIdx();
