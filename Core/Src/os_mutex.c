...
static OS_Result_t prv_Lock(OS_Mutex_t *mutex, uint32_t u32TimeoutTicks, uint8_t u8NonBlocking)
{
    ...
    for (;;)
    {
        ...
        {
            OS_Trace_Record2(OS_TRACE_EVT_MTX_LOCK_TRY, mutex->u8TraceId, u8Idx);
        }
        ...
        if (mutex->u8Locked == 0u)
        {
            ...
            OS_Trace_Record2(OS_TRACE_EVT_MTX_LOCK_OK, mutex->u8TraceId, u8Idx);
        ...
        if (u32TimeoutTicks != OS_WAIT_FOREVER)
        {
            ...
            if (u32Remaining == 0u)
            {
                OS_Trace_Record2(OS_TRACE_EVT_MTX_LOCK_TIMEOUT, mutex->u8TraceId, u8Idx);
                OS_vExitCritical(u32PriMask);
                return OS_TIMEOUT;
            }
            mutex->u32WaitMask |= u32Bit;
            OS_Trace_Record3(OS_TRACE_EVT_MTX_LOCK_BLOCK, mutex->u8TraceId, u8Idx, u32Remaining);
            Scheduler_vBlockCurrentTask(u32Remaining, MUTEX_SYSVIEW_CAUSE);
        }
        else
        {
            mutex->u32WaitMask |= u32Bit;
            OS_Trace_Record3(OS_TRACE_EVT_MTX_LOCK_BLOCK, mutex->u8TraceId, u8Idx, OS_WAIT_FOREVER);
...
void OS_Mutex_Unlock(OS_Mutex_t *mutex)
{
    ...
    if (mutex->u8Locked != 0u && mutex->u8OwnerTaskId == g_pCurrentTask->u8TaskId)
    {
        mutex->u8Locked      = 0u;
        mutex->u8OwnerTaskId = 0u;
        OS_Trace_Record2(OS_TRACE_EVT_MTX_UNLOCK_OK, mutex->u8TraceId, u8Idx);
    ...
    else
    {
        ...
        OS_Trace_Record2(OS_TRACE_EVT_MTX_UNLOCK_DENIED, mutex->u8TraceId, u8Idx);
    }
